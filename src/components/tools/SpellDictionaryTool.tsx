...
export const SpellDictionaryTool: React.FC = () => {
  ...
  return (
    <section id="bangla-spell-dictionary-container" className="space-y-4 sm:space-y-6">
      ...
      <div className="bg-white rounded-[2rem] card-elevation p-5 sm:p-8 space-y-3.5 sm:space-y-4">
        ...
        <div className="space-y-2">
          <label htmlFor="spell-text-input" className="font-bold text-slate-700 text-xs sm:text-sm">
        ...
        {/* Spell Check Results */}
        {spellResult.hasMistakes ? (
          <div className="bg-amber-50/70 border border-amber-200 rounded-[2rem] p-3.5 sm:p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 sm:gap-3">
              {spellResult.mistakes.map((m, idx) => (
                <div key={idx} className="bg-white p-3 sm:p-3.5 rounded-[1rem] border border-amber-200 shadow-sm flex items-center justify-between gap-3">
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-[2rem] p-3.5 sm:p-4 flex items-center gap-3 text-purple-800 font-semibold text-xs sm:text-sm">
      ...
      <div className="bg-white rounded-[2rem] card-elevation p-5 sm:p-8 space-y-3.5 sm:space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/60 pb-4">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-xs sm:text-sm bg-slate-50 border border-slate-300/60 rounded-[1rem] outline-none focus:border-slate-900 focus:bg-white"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          {filteredDict.map((item, idx) => (
            <div key={idx} className="bg-slate-50/70 p-3.5 sm:p-4 rounded-[2rem] border border-slate-300/60 hover:border-purple-300 transition-all space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <h3 className="text-base sm:text-lg font-bold text-slate-900 font-bangla">{item.wordBn || item.word}</h3>
                  <span className="text-[11px] sm:text-xs bg-slate-200 text-slate-700 font-semibold px-2 py-0.5 rounded-md">
                    {item.posBn || item.pos}
                  </span>
                </div>
